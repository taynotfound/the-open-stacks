...
(async () => {
  ...
  for (const [, item] of rss.matchAll(/<item>([\s\S]*?)<\/item>/g)) {
    ...
    await upsert(db, { slug, title, author: 'Anarchist News', desc: body.slice(0, 300), body, source: link, sourceName: 'anarchistnews.org', category: 'anarchist-news', language: 'eng', tags: ['anarchism', 'news'], hasBody: true, atRisk: false, cover: '', files: [], images: [], links: [], state: 'active', path: '', pageType: 'external' });
    inserted++;
  }
